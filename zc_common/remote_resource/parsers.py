...
class ResourceDataParser(object):
    """
    For events that will create resources in other services, this parser will extracts attributes from the event
    metadata so that DRF serializers can work.
    """
    @staticmethod
    def parse_attributes(data):
        return utils.format_keys(data.get('attributes'), 'underscore') if data.get('attributes') else dict()
    ...
    def parse(self, raw):
        ...
        parsed_data.update(self.parse_attributes(data))
        parsed_data.update(self.parse_relationships(data))
        parsed_data.update(self.parse_metadata(raw))
        return parsed_data


def parse_resource_data_from_metadata(metadata):
    parser = ResourceDataParser()
